...
<https://github.ubc.ca/MDS-2022-23/DSCI_521_lab4_ritish01>
...
https://ritisha2000.github.io/repo_521/
...
```{r}
print("Output of new code chunk")
```
...
The following code chunk produces an warning because of division by 0.
The warning does not show up after knitting because the chunk option
"warning" is set to FALSE.

```{r warning = FALSE}
n = 2
d = 0
if (d == 0){
  warning("Division by zero warning message")
} else {
  print(n/d)
}
```
...
- The option "highlight = FALSE" removes the code highlights that would normally appear in R code. In the following code, highlighting is removed from the constants. 
- The option "fig.cap" adds a caption to the plot created by the code
chunk.

```{r highlight = FALSE, fig.cap = "Graph of x^2"}
x <- -10:10
y <- x^2
plot(x, y)
```
...
```
Link to HTML output: https://github.ubc.ca/MDS-2022-23/DSCI_521_lab4_ritish01/blob/master/lab4.html  
Link to PDF output: https://github.ubc.ca/MDS-2022-23/DSCI_521_lab4_ritish01/blob/master/report.pdf
